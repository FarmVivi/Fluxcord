...
| `core/command/parser/{Slash,Text,Console}CommandParser.java` + `CommandParser` | `canParse(event)` / `isCommandInvocation(event)` / `extractCommandName` / `parse(event, command)` → `CommandContext`. `ConsoleCommandParser` wraps a synthetic `parser/event/ConsoleCommandEvent`. Text and console share `parser/PositionalArguments` (`split` with quotes, `assign` tokens → options in declaration order: trailing STRING absorbs the rest, a failed required option throws, a failed optional one is treated as omitted and its token moves to the next option). |
