...
- 2026-09-22: For a plugin module always use `mvn -o -pl plugins/music-plugin -am test`. Without `-am`, Maven resolves `fluxcord-api` from `~/.m2` and the build can fail with `cannot access PluginDataStorageAdapter` / `cannot find symbol: method getLanguage()` even though the installed jar contains those classes.
