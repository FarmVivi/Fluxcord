...
4. Health check: `curl -s localhost:8081/health` (port from `HEALTH_PORT`).
...
## Learnings
- 2026-09-19: Initial version; commands taken from CLAUDE.md / root pom (`defaultGoal` = `clean package`). No timings measured yet — record real ones here.
- 2026-09-19: No GitHub workflow runs the test suite (docker build, sonar `-DskipTests`, release only). Local verification is currently the only safety net.
