...
| `api/plugin/AbstractPlugin.java` | base class; `onLoad` keeps the context, logger, event manager and Discord API; `getCommands()/getPermissions()/getLanguage()/getStorage()/getBinaryStorage()` delegate to the context (a mocked context must stub them — see `TemplatePluginTest`). |
...
`loadPlugin` per jar: new `PluginClassLoader` → instantiate `main` → fire `PluginLoadingEvent` → build `PluginConfiguration` + `PluginContextImpl` → `plugin.setLifecycle(LOADED)`; `plugin.onLoad(ctx)` → `pluginConfig.initializeMigration(plugin)` → `LanguageFiles.loadJar` + `loadFolder(plugins/<id>/lang)` into namespace = plugin id (the manager lower-cases namespaces) → fire `PluginLoadedEvent`.
...
- `PluginContext` no longer exposes the shared `LanguageManager`/`DataStorageManager`/`BinaryStorageManager`/`PermissionManager`/`CommandService` (removed 2026-09-20 after P5); it keeps `EventManager`, `DiscordAPI`, `AudioService`, `Configuration`, `PluginLoader` and the five scoped views. Escape hatches: `getCommands().getCommandService()`, `getLanguage().getLanguageManager()`. `AbstractPlugin` fields are down to `context`, `logger`, `eventManager`, `discordAPI`; its scoped getters are named like the context's (`getCommands()` …, the old `getPluginXxx()` names are gone).
