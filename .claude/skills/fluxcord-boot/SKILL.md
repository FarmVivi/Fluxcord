...
- `Fluxcord` keeps only the statics read elsewhere: `NAME`, `VERSION`, `PRODUCTION = !VERSION.contains("-SNAPSHOT")` (from Maven-filtered `project.properties`; used by `VersionCommand`, `JDADiscordAPI` default activity, health `/version`). `main`: `CoreConfiguration(baseDir/config.yml)` → `CoreSettings.from(config, baseDir)` (any `ConfigurationException` → exit 1) → `new FluxcordRuntime(baseDir, settings, new JDADiscordAPI(settings.token()))` → shutdown hook (`runtime::stop`, thread `Fluxcord-Shutdown`) → `startHealthServer(HEALTH_PORT|8081)` → `start()` (failure → `stop()` + exit 1) → `awaitShutdownRequest()` → `stop()` → `System.exit(0)`. `System.exit` exists only in `main` and the static initialiser.
- `FluxcordRuntime(baseDir, CoreSettings, DiscordAPI[, InputStream consoleInput])` constructor (no I/O towards Discord): `plugins/` folder → `SimpleEventManager` → `SimpleLanguageManager(settings.defaultLocale(), events)` + `LanguageFiles.loadFolder(…, "core", baseDir/lang)` → `StorageFactory(settings.dataStorage())` / `BinaryStorageFactory(settings.binaryStorage())` → `SimplePermissionManager(events, storage, settings.operators())` → `AudioServiceImpl(events, settings.audio())` → `SimpleCommandService(..., settings.commands(), storage)` + `setShutdownHandler(runtime::requestShutdown)` → `ConsoleCommandService` → `PluginManager(all services)`. Throws on a storage backend failure.
...
`YamlConfiguration` (snakeyaml, dotted keys `getString/getInt/getBoolean/getStringList`, `set`, `save`, `reload` — a YAML syntax error surfaces as `ConfigurationException`) ← `CoreConfiguration` adds `config_version` + `validateConfiguration()` + migration (legacy files without version are treated as 0). **`CoreSettings.from(config, baseDir)`** (B3) reads every core key once into records — `token`, `defaultLocale`, `Commands(defaultPrefix, help, version, shutdown, perm)`, `operators`, `DataStorage(type FILE|DB, fallback, fileFolder, debounceMs, Database(url, username, password, tablePrefix, maxPoolSize?, autoCommit?))`, `BinaryStorage(type FILE|S3, fallback, fileFolder, S3(...))`, `AudioSettings` — validates (token placeholder, unknown backend, DB URL, S3 required keys) and resolves relative folders against `baseDir`. Services take the sub-record; the key strings exist only in `CoreSettings` and the default `config.yml`. To add a key: `config.yml` + `CoreSettings.from` + the record + `CoreSettingsTest`. Plugin configs reuse `YamlConfiguration` via `PluginConfiguration` (see `fluxcord-plugin-system`). Tests: `CoreConfigurationTest`, `CoreSettingsTest`.
...
- `Permission(name, description, PermissionDefault)`; plugins register in `onPreEnable` via `getPermissions().registerPermission(...)` (name convention `<pluginId>.<node>`, see `MusicPlugin.permissionKey`).
...
- `PluginConfiguration` is still cwd/`plugins.dir`-relative (storage folders are resolved against `baseDir` since B3).
