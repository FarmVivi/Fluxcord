...
- [x] **P5 — `PluginContext` exposes plugin-scoped adapters.** Done 2026-09-20: `getCommands()`, `getPermissions()`, `getLanguage()`, `getStorage()`, `getBinaryStorage()` on `PluginContext` (built by `PluginManager` from the descriptor id/name, before `onLoad`); `AbstractPlugin` uses them (falls back to building its own for contexts that return null, e.g. mocks). Old shared-manager getters kept. Adapters gained id-based constructors.
