...
- `AbstractPlugin` is what real plugins extend; `PluginContext` (P5) hands out the plugin-scoped views `getCommands()`, `getPermissions()`, `getLanguage()`, `getStorage()`, `getBinaryStorage()` (namespaced by plugin id) and `AbstractPlugin` exposes the same instances with the same names. The shared managers are only reachable through the adapters (`getCommands().getCommandService()`, `getLanguage().getLanguageManager()`) when a plugin must leave its scope.
